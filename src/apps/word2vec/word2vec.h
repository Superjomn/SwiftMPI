...
public:
  ...
  virtual void apply_push_value(const w2v_key_t &key, param_t &param,
    ...
    // LOG (INFO) << "apply push  " << key << "  param:" << param << "grad  " <<
    // push_val.h_grad << "  " << push_val.v_grad;
...
public:
  ...
  size_t num_words() noexcept { return _num_words; }
...
public:
  ...
  float train_iter(const std::string &path) noexcept {
    ...
    SpinLock spinlock;
    Instance ins;
    AsynExec::task_t handler = [this, &file, &file_mut, &line_reader, &line_id,
                                &line_count, &spinlock] {
      std::string line;
      char *cline;
      Instance ins;
      ...
      while (true) {
        ...
        if (line_id % _batchsize == 0)
          RAW_LOG_INFO("train status:\t%f\t%d/%d", (float)line_id / nlines,
                       line_id, nlines);
    ...
    while (true) {
      line_count = 0;
      if (_minibatch.gather_keys(file, _line_id, _batchsize) < 5)
        break;
      _minibatch.pull();
      async_exec(_nthreads, handler, global_channel());
      // LOG (INFO) << "... push()";
